...
| PLR-008 | P0     | ALL      | 部分实现 | 播放错误返回来源、阶段、错误码、可重试性和用户建议                   | 文件损坏、网络中断、解码失败、设备丢失均可区分 |
...
| PLR-102 | P1     | ALL      | 部分实现 | 随机、单曲循环、列表循环和队列编辑                                   | 模式切换及队列边界行为有自动化测试             |
...
| PLG-005 | P0     | ALL  | 部分实现 | 支持浏览、搜索、安装、启用、停用、配置、升级、回滚和卸载                            | UI 与服务状态一致，重启后保持                                                                                                     |
| PLG-006 | P0     | ALL  | 部分实现 | 权限按能力授予，安装前展示，运行时可撤销                                            | 撤销后插件访问立即失败且返回 PermissionDenied                                                                                     |
| PLG-007 | P0     | ALL  | 部分实现 | 官方原生与社区沙箱形成双通道信任；桌面高级授权有持续警告和审计记录                  | 默认不能静默运行社区原生代码                                                                                                      |
...
| PLG-011 | P0     | ALL  | 部分实现 | 插件配置依据 JSON Schema/等价声明渲染并持久化                                       | 类型、范围、默认值、敏感字段和迁移均通过                                                                                          |
...
| DST-002 | P0     | ALL  | 部分实现 | 保留发布者原文件，并允许附加 AAC/Opus 等兼容 rendition                              | 客户端按能力、质量和网络策略选择版本         |
...
| COM-006 | P0     | ALL  | 部分实现 | Policy 动作为 warn、demote、hide、block、revoke，并声明目标、原因、证据、范围和到期 | 搜索、详情、精确打开三种入口均应用策略       |
...
| UI-004 | P0     | ALL  | 部分实现 | 发布页完成元数据、权利声明、rendition、签名、Pin 和副本健康度流程         | 不完整发布不能进入公共 Feed          |
...
| UI-101 | P1     | ALL  | 部分实现 | 插件 ControlPanel 根据参数 Schema 渲染滑块、枚举、开关、仪表和预设        | 不允许任意代码注入 UI                |
...
| SEC-009 | P0     | ALL  | 部分实现 | 支持本地屏蔽、社区策略、举报和来源解释               | 被过滤内容可查询决策来源                 |
...
| 社区发现       | 双 Feed、可禁用签名启动源、URI/CID/IPNS 导入、换钥/撤销与加密举报        | 启动源尚无远端 Feed；缺相机扫码和 Feed 快照/压缩                   |
