...
| AGR-005 | P0     | ALL      | 本机通过 | 实时线程禁止阻塞锁、文件/网络 I/O、日志和不可控系统调用                | 测试构建可检测分配和阻塞违规               |
...
| AGR-013 | P0     | ALL      | 本机通过 | 图和节点状态可序列化，插件升级支持状态迁移                             | 升级、降级和迁移失败回滚场景通过           |
...
| DST-003 | P0     | ALL  | 本机通过 | 发布流程包含编辑、校验、CID 生成、签名、Pin、Feed 事件和副本检查                    | 发布完成后另一节点可解析并播放               |
...
| ID      | 优先级 | 平台 | 状态     | 要求                                                                  | 验收                                         |
| ------- | ------ | ---- | -------- | --------------------------------------------------------------------- | -------------------------------------------- |
| API-001 | P0     | ALL  | 本机通过 | 所有上层能力先定义传输无关服务契约，再映射到 FFI/IPC、HTTP 或 JS/WASM | 同一契约测试可对不同传输适配器运行           |
...
| NFR-004 | P0     | ALL  | 本机通过 | 实时 process 在所选块 deadline 内完成，发布构建持续记录超时       |
...
| 音乐分发       | Manifest/rendition、签名 Feed、原子下载、持久优先级调度与任务恢复；收藏协助 Pin 与第三方 Pin 服务 | 跨节点 Feed→曲库→播放闭环已落地（测试网关模拟远端可达性）；七端实测待补 |
