...
| Rust tests | 通过（238） | 单元、动态库 FFI、本地 HTTP/CAS/加密/P2P 集成、传输流端点、网络类别与蜂窝额度、策略撤销自动停用与防回滚、收藏协助 Pin、发布者关注与策略本地覆盖，workspace/all targets/all features |
...
| Flutter tests | 通过（54） | provider/model/widget、Rust 播放/输出会话/节点 FFI、控制面 SSE 解析/真实 HTTP 流/Provider 缺口重读、边下边播代理链路、播放页来源/缓冲/传输状态、关注发布者与策略覆盖 mutation、曲库统一同步、错误本地化 |
