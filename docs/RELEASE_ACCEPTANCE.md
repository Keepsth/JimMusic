...
| Flutter tests | 通过（90） | provider/model/widget、Rust 播放/输出会话/节点 FFI、控制面 SSE 解析/真实 HTTP 流/Provider 缺口重读、边下边播代理链路、播放页来源/缓冲/传输状态、关注发布者与策略覆盖 mutation、曲库统一同步、网络曲目播放入口、Rendition 选择、音乐目录设置、发布向导（含多 rendition 编辑）、三入口策略应用、错误本地化、发布者索引、离线队列提示、播放模式与队列边界、声明式配置控件与敏感字段、操作取消 |
...
| P0 追踪完整性 | 通过 | 134/134 已映射：本机通过 91、部分实现 31、缺失 0、待外证 12；“无缺失”不等于已满足跨平台 DoD |
